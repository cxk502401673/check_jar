...
public class InsertTable {
    ...
    //update
    public  int CK_update(Connection conn, String sql,Object...object){
    ...
    //级联开关控制
    public  int CK_StatisticFinishCheck(Connection conn, String sql,Object...object){
        return SQLDetailDao.sqlUpdate(conn, sql,object);
    }
    ...
    //更新任务信息
    public  int CK_CreateCheckConfig(Connection conn, String sql,List<Map<String,Object>> list){
        int row=0;
        try {
            PreparedStatement statement =conn.prepareStatement(sql);
            ...
            for (Map<String,Object> map:list) {
                Object id =map.get("id");
                Object checkover =map.get("checkover");
                Object updatetime =map.get("updatetime");
                Object checkitems =map.get("checkitems");
                Object registermode =map.get("registermode");
                Object depthcheckstr =map.get("depthcheckstr");
                Object isonlyhist =map.get("isonlyhist");
                Object organizename =map.get("organizename");
                Object checkstate =map.get("checkstate");
                Object iprange =map.get("iprange");
                Object checkmode =map.get("checkmode");
                Object checkgroupid =map.get("checkgroupid");
                //添加参数，级联下发的任务传递值默认都是1
                Object[] objects = {
                        organizename, iprange, registermode, checkmode, checkover, checkitems, checkstate, depthcheckstr, isonlyhist,
                        1};
                for (int i = 0; i < objects.length; i++) {
                    statement.setObject(i+1, objects[i]);
                }
            ...
            int[] rows = statement.executeBatch();
            logger.info(  "CK_CreateCheckConfig-->"+Arrays.toString(rows));
            conn.setAutoCommit(true);//1,首先把Auto commit设置为false,不让它自动提交
            //conn.commit();
            statement.close();
        } catch (Exception e) {
